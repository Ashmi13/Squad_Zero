"""Calendar event endpoints — Member 5"""
...
async def update_event(event_id: str, event: CalendarEventUpdate, current_user: dict = Depends(get_current_user)):
    supabase = get_supabase().service_client
    user_id = current_user.get("sub")
    ...
    res = supabase.table("calendar_events").update(update_data).eq("id", event_id).eq("user_id", user_id).execute()
    if not res.data:
...
async def delete_event(event_id: str, current_user: dict = Depends(get_current_user)):
    supabase = get_supabase().service_client
    user_id = current_user.get("sub")
    res = supabase.table("calendar_events").delete().eq("id", event_id).eq("user_id", user_id).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="Event not found")
