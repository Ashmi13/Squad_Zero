...
class AIService:
    ...
    def _build_prompt(
        ...
        # Strip the "--- filename.ext ---" markers FileProcessor inserts between
        # uploaded files. Left in, the model sometimes anchors on them and asks
        # about the file itself ("What type of document is this?") instead of
        # its actual subject matter — especially when a file yields little
        # extractable text.
        cleaned = re.sub(r"\n*--- .+? ---\n*", "\n\n", text).strip()
        truncated = self._smart_truncate(cleaned)
        return f"""Generate exactly {num_questions} {difficulty} level quiz questions based on the study material below.
...
- Every question must test understanding of the SUBJECT MATTER in the study material.
- NEVER ask about the document/file itself — its format, type, filename, length, how it was uploaded, or that it "was provided". If you catch yourself writing a question like "What type of document is this?", discard it and write a real content question instead.
- If the study material is too sparse or generic to support {num_questions} distinct, substantive questions, generate as many genuinely meaningful ones as the content allows rather than padding with meta questions.
