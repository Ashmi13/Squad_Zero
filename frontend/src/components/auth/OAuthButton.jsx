 ...
 * Handles Google and GitHub authentication redirects
 * Styled according to Figma design specifications
 */
export const OAuthButton = ({ provider, disabled = false }) => {
  const handleOAuthClick = () => {
    if (disabled) return;

    const authUrl =
      provider === 'google'
        ? `${config.apiBaseUrl}${config.oauth.googleAuthUrl}`
        : `${config.apiBaseUrl}${config.oauth.githubAuthUrl}`;
