...
export default function CategoryModal({ open, onClose, onSave, initial }) {
  ...
  // fill in existing values when editing, or reset for a new list
  useEffect(() => {
    if (!open) return;
    setName(initial?.name   || '');
    setIcon(initial?.icon   || 'default');
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth
      ...
      <DialogContent sx={{ pt: 3, display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        {/* preview icon + name field side by side */}
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <Box sx={{ width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center',
                     bgcolor: `${color}18`, borderRadius: 2, border: `2px solid ${color}`, flexShrink: 0 }}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
            {ICONS.map(i => (
              <Box key={i} onClick={() => setIcon(i)}
                sx={{ width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center',
