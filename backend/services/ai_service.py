...
class AIService:
    ...
    # Primary model — fast, cheap, good at JSON
    MODEL = "google/gemini-2.5-flash"
    # Fallback model
    FALLBACK_MODEL = "openai/gpt-4o-mini"
    # Second fallback — always free
    FREE_FALLBACK_MODEL = "mistralai/mistral-7b-instruct:free"
    ...
    async def generate_questions(
        ...
        # Scale tokens: ~120 tokens per question for MCQ, more for long-answer
        tokens_per_q = 200 if question_type in ("long_answer", "mixed") else 130
        max_tokens = max(4096, num_questions * tokens_per_q + 512)
        ...
        for model_name in (self.MODEL, self.FALLBACK_MODEL, self.FREE_FALLBACK_MODEL):
            try:
                ...
                print(f"⚠️  {model_name} returned unparseable response (first 300 chars): {raw[:300]}")
            except Exception as err:
                print(f"⚠️  Model {model_name} failed: {err!r}. Trying next...")

        print("⚠️  All models failed — returning mock questions. Check OPENROUTER_API_KEY and model availability.")
...
"""
    ...
    def _repair_truncated_json(text: str) -> str:
        """Best-effort repair of a JSON array truncated mid-stream.

        Strips the incomplete trailing object and closes the array so
        json.loads() can at least recover the complete questions.
        """
        # Find the last *complete* object — ends with } possibly followed by
        # whitespace/comma before the next [ or end of input
        last_complete = max(text.rfind('},'), text.rfind('}\n'))
        if last_complete != -1:
            return text[:last_complete + 1] + "]"
        # If no complete object at all, return as-is and let caller handle
        return text
    ...
    def _parse_response(self, response: str, question_type: str) -> Optional[list]:
        ...
        try:
            ...
            json_str = cleaned[start:end] if end > 0 else cleaned[start:]

            # Try parsing as-is first; if it fails try repairing truncation
            raw_questions = None
            for attempt in (json_str, self._repair_truncated_json(json_str)):
                try:
                    raw_questions = json.loads(attempt)
                    break
                except json.JSONDecodeError:
                    continue

            if not raw_questions:
                print(f"❌ JSON decode failed even after repair. Snippet: {json_str[:300]}")
                return None
